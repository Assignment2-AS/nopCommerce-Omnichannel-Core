...
The diagrams below document the observable runtime behaviour during normal operation, WMS degradation, automatic recovery, and cross-channel stock sync.
...
sequenceDiagram
    ...
    note right of OutboxDB: INSERT Order<br/>INSERT OutboxMessage order.placed<br/>COMMIT atomic
    ...
    WireMock-->>OrderSync: 200 OK - received by ERP
    OrderSync->>WireMock: POST /api/wms/orders - WMS
    WireMock-->>OrderSync: 200 OK - received by WMS
...
## Diagram 1b — Stock sync flow (WMS to nopCommerce)
...
sequenceDiagram
    ...
    Script->>RabbitMQ: Publish wms.stock.update - sku AP_MBP_13 delta +50
    ...
    NopCatalog-->>StockSvc: Stock updated 0 to 50
...
`WmsStockSyncService` runs inside the nopCommerce plugin. It resolves the product by SKU and applies the delta via `AdjustInventoryAsync`, which records a proper stock history entry. Applying the same absolute quantity twice is safe — the delta is zero on the second call.
...
sequenceDiagram
    ...
    OrderSync->>DLQ: Publish to order.placed.dlq with CorrelationId
    note right of OrderSync: After 5 consecutive failures across messages<br/>Circuit breaker OPENS<br/>Subsequent WMS calls return stale cached value
...
sequenceDiagram
    ...
    Operator->>WireMock: restore-wms.sh - DELETE fault mapping
    note right of WireMock: Normal stub reactivated<br/>POST /api/wms/orders returns 200 OK

    note over OrderSync: Circuit breaker HALF-OPEN after 30s
    note over OrderSync: New order triggers probe request
    OrderSync->>WireMock: POST /api/wms/orders - probe
    WireMock-->>OrderSync: 200 OK
